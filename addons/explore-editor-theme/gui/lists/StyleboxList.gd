...
const ITEM_SCENE := preload("./StyleboxListItem.tscn")
const StyleboxListItem := preload("./StyleboxListItem.gd")

var _selected_item_index: int = -1
var _item_node_pool: Array[StyleboxListItem] = []

@onready var _item_list: GridContainer = %List
...
func clear() -> void:
	...
	for item_node: StyleboxListItem in _item_list.get_children():
...
func add_item(stylebox_name: String, resource: StyleBox) -> void:
	var item_node: StyleboxListItem = _item_node_pool.pop_back()
	if not item_node:
		item_node = ITEM_SCENE.instantiate()

	item_node.stylebox_name = stylebox_name
	item_node.stylebox_resource = resource
...
func get_item_name(item_index: int) -> String:
	...
	var item_node: StyleboxListItem = _item_list.get_child(item_index)
	return item_node.stylebox_name


func get_item_stylebox(item_index: int) -> StyleBox:
	if item_index < 0 || item_index >= _item_list.get_child_count():
		return null

	var item_node: StyleboxListItem = _item_list.get_child(item_index)
	return item_node.stylebox_resource


func _select_item_node(item_node: StyleboxListItem) -> void:
	...
	if _selected_item_index >= 0 && _selected_item_index < _item_list.get_child_count():
		var current_node: StyleboxListItem = _item_list.get_child(_selected_item_index)
